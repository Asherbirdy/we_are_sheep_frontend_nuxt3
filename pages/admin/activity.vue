      ...
      <UForm ref="form" :state="state">
        <div class="flex gap-2 align-end">
          <UFormGroup label="年" name="selectedYear">
            <USelectMenu v-model="state.selectedYear" :options="yearsOptions" placeholder="年" class="w-20"
              @change="onChangeYear" />
          </UFormGroup>
          <UFormGroup label="月" name="selectedMonth">
            <USelectMenu v-model="state.selectedMonth" :options="monthsOptions" placeholder="月" class="w-20"
              @change="onChangeMonth" />
          </UFormGroup>
          <UButton @click="onSearch">查詢</UButton>
          <UButton label="新增活動" @click="onCreate" />
        </div>
      </UForm>
    </div>

    <UTable :rows="activityList" :columns="columns">
      <template #actions-data="{ row }">
        <UButton label="編輯" class="mr-2" @click="onEdit(row)" />
        <UButton label="刪除" @click="onDelete(row._id)" />
      </template>
    </UTable>
    <!-- <UButton label="新增活動" /> -->
      ...
      <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
        <template #header>
          <div class="flex items-center justify-between">
            <h3 class="text-base font-semibold leading-6 text-gray-900 dark:text-white">{{ modalType === "create" ?
              "新增活動" : "編輯活動" }}</h3>
        ...
        <div class="max-w-lg mx-auto space-y-6 p-8 bg-white rounded-lg shadow-lg">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <UInputMenu v-model="createState.selectedYear" :options="yearsOptions" @change="onChangeCreateYear" :disabled="modalType === 'edit'" />
            </div>

            <!-- 月份 -->
            <div>
              <label for="month" class="block text-sm font-medium text-gray-700">月份</label>
              <UInputMenu v-model="createState.selectedMonth" :options="monthsOptions" @change="onChangeCreateMonth" :disabled="modalType === 'edit'" />
            </div>

            <!-- 日期 -->
            <div>
              <label for="day" class="block text-sm font-medium text-gray-700">日期</label>
              <UInputMenu v-model="createState.selectedDay" :options="daysOptions" @change="onChangeCreateDay" :disabled="modalType === 'edit'" />
            </div>

            <!-- 時間 -->
            <div>
              <label for="time" class="block text-sm font-medium text-gray-700">時間</label>
              <input id="time" v-model="createState.selectTime" type="time"
                class="mt-1 block w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm" :disabled="modalType === 'edit'" />
            </div>

            <!-- 標題 -->
            <div>
              <label for="title" class="block text-sm font-medium text-gray-700">標題</label>
              <input id="title" v-model="createState.FormTitle" type="text"
            ...
            <div>
              ...
              <textarea id="content" v-model="createState.FormContent" rows="4"
                class="mt-1 block w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm"
                placeholder="請輸入內容" />
            </div>
          </div>
